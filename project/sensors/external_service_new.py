...
class ExternalSensorService:
    ...
    def send_sensor_to_external_server(self, sensor):
        ...
        """
        try:
            # Prepare sensor data in simplified format as requested
            sensor_data = {
                "id": self._sanitize_string(sensor.sensor_id),
                "type": self._sanitize_string(sensor.parameter),  # Use parameter as type (humedad, temperatura, etc.)
                "0.0": {  # Default value attribute
                    "type": "Float",
                    "value": 0.0,
                    "metadata": {}
                }
    ...
    def update_sensor_value(self, sensor, value, unit="units"):
        ...
        Args:
            ...
            unit: The unit of measurement (not used in simplified format)
